...
export default async function CampersDashboard() {
  ...
  if (!error && campersData) {
    campers = campersData.map((c: any) => ({
      ...
      name: c.name,
      status: c.statuses?.name || 'New',
      created_at: c.created_at,
      updated_at: c.updated_at,
      // Default to created_at for last activity for now
      last_activity: new Date(c.updated_at || c.created_at).toLocaleDateString(),
    }));
  } else {
    console.error("Error fetching campers:", error);
    // For local dev/testing before DB is seeded
            ...
            {campers.length === 0 ? (
              ...
              campers.map((camper) => (
                <TableRow key={camper.id} className="cursor-pointer group">
                  ...
                  <TableCell>
                    <Link href={`/campers/${camper.id}`} className="block">
                      <Badge variant={getStatusVariant(camper.status as any)}>
                        {camper.status}
                      </Badge>
                    </Link>
                  </TableCell>
