...
export default async function SessionBStaffPage() {
  ...
  try {
    ...
    if (data && data.length > 0) {
      ...
      staff = data.map((u: any) => ({
        ...
        role: u.role === 'admin' ? 'Head Administrator' : 'Session B Counselor',
        email: u.email || 'N/A',
        status: u.active !== false ? 'Active' : 'Inactive',
      }))
    }
  } catch (err) {
    console.error('Failed to load Session B staff:', err)
  }
