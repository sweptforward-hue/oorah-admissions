...
function createServiceAccountJwt(clientEmail: string, privateKey: string): string {
  ...
  // Format private key properly if escaped newlines exist
...
/**
 * Retrieves a valid Google OAuth2 access token via Service Account JWT or Refresh Token.
 */
export async function getAccessToken(): Promise<string> {
  ...
  const serviceKey = process.env.GOOGLE_PRIVATE_KEY || process.env.GOOGLE_SERVICE_ACCOUNT_KEY
  ...
  if (serviceEmail && serviceKey) {
    const jwt = createServiceAccountJwt(serviceEmail, serviceKey)
    ...
    // Fallback/mock token if no credentials set in dev/test environment
...
export async function uploadKidMediaAsset(params: UploadKidMediaAssetParams): Promise<{
  ...
  try {
    ...
    if (category === 'Photos') {
      ...
      if (error) throw new Error(`Database error saving photo metadata: ${error.message}`)
      ...
      if (error) throw new Error(`Database error saving voice note metadata: ${error.message}`)
      dbRecord = data as DbMediaRecord
      ...
      if (error) throw new Error(`Database error saving document metadata: ${error.message}`)
      dbRecord = data as DbMediaRecord
      ...
      if (error) throw new Error(`Database error saving transcript metadata: ${error.message}`)
      dbRecord = data as DbMediaRecord
    } else if (category === 'Chat Exports') {
      const { data, error } = await supabase
        .from('chat_exports')
        .insert({
          kid_id: kidId,
      ...
      if (error) throw new Error(`Database error saving chat export metadata: ${error.message}`)
      dbRecord = data as DbMediaRecord
    }

    // 4. Audit Log
    await supabase.from('audit_log').insert({
      user_id: uploadedBy || null,
      action: 'media_upload',
      entity_type: category.toLowerCase().replace(/\s+/g, '_'),
      entity_id: dbRecord?.id || null,
      metadata: {
        filename,
        drive_file_id: driveFile.id,
        category,
 ...
 */
export async function getFileMetadata(fileId: string): Promise<GoogleDriveFile> {
  const token = await getAccessToken()
  ...
  if (!res.ok) {
    const errText = await res.text()
    throw new Error(`Failed to fetch file metadata: ${res.status} ${errText}`)
  }
...
/**
 * Fetches the file stream/media from Google Drive API forwarding Range headers.
 */
