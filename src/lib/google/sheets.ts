...
export function buildAutoFitRequests() {
  return REQUIRED_TAB_NAMES.map((_, index) => ({
...
export async function createMultiTabSpreadsheet(
  title: string,
  data: ExportData = {},
  accessToken?: string
): Promise<{ spreadsheetId: string; spreadsheetUrl: string; payload: any }> {
  const payload = buildSheetPayload(title, data)
  const autoFitRequests = buildAutoFitRequests()
  ...
  if (accessToken) {
    try {
      ...
      await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}:batchUpdate`, {
        ...
        body: JSON.stringify({ requests: autoFitRequests }),
      })
      ...
      console.warn('Google Sheets API execution failed, returning structured spreadsheet spec:', err)
    }
  }

  // Fallback / local representation when access token is not provided or API call is offline
